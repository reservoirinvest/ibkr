...
def target_snp(ib, df_sized, blacklist, assignment_limit):
    '''Args:
        (ib) as connection object
        (df_sized) picked up from sized pickle
        (blacklist) as list of blacklisted stocks
        (assignment_limit) snp_assignment_limit from variables.json
       Returns:
    ...
    # get the options
    df_opt = df_sized.assign(und_remq=assignment_limit/(df_sized.lot*df_sized.undPrice)) # remaining quantities in entire snp
